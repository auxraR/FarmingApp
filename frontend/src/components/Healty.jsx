    ...
    return (
      <div className="grid grid-cols-7 gap-2 mt-4">
        {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
          <div key={day} className="text-center text-[10px] text-gray-500 font-bold uppercase">{day}</div>
        ))}
        ...
        {daysArray.map(day => {
          ...
          return (
            <div 
              key={day} 
              className={`relative flex flex-col items-center justify-center p-2 rounded-xl h-12 border ${hasVaccine ? 'bg-red-500/20 border-red-500/50 cursor-pointer group' : 'bg-black/20 border-white/5'}`}
            >
              <span className={`text-sm font-bold ${hasVaccine ? 'text-red-400' : 'text-gray-400'}`}>{day}</span>
              ...
              {hasVaccine && (
                <div className="absolute bottom-full mb-2 hidden group-hover:block w-max bg-black text-white text-[10px] px-3 py-1 rounded-lg z-10 border border-white/10">
  ...
  return (
    <div className="space-y-6 text-white p-4">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2"><Activity className="text-ganadero-active"/> Health & Weight</h1>
        ...
        <div className="relative w-full md:w-96">
          ...
          <input 
            ...
            className="w-full bg-[#1a1c26] border border-white/10 rounded-xl py-3 pl-10 pr-4 outline-none focus:border-ganadero-active transition-all shadow-lg"
            value={searchTerm}
          ...
          {searchResults.length > 0 && (
            <div className="absolute top-full mt-2 w-full bg-[#1a1c26] border border-white/10 rounded-xl overflow-hidden shadow-2xl z-50">
      ...
      {!selectedAnimal ? (
        <div className="flex flex-col items-center justify-center h-64 border-2 border-dashed border-white/10 rounded-3xl bg-[#1a1c26]/50">
          <Syringe size={48} className="text-gray-600 mb-4" />
          <p className="text-gray-400 text-lg">Search and select an animal to view health records</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in zoom-in duration-300">
          ...
          <div className="lg:col-span-5 space-y-6">
            ...
            {/* ANIMAL SUMMARY CARD */}
            <div className="bg-[#1a1c26] p-6 rounded-3xl border border-white/10 shadow-xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-ganadero-active/10 rounded-bl-full -z-0"></div>
              <p className="text-sm font-mono text-ganadero-active mb-1 relative z-10">ID: #{selectedAnimal.id}</p>
              <h2 className="text-3xl font-black text-white relative z-10">{selectedAnimal.nombre}</h2>
            ...
            {/* WEIGHT CONTROL FORM */}
            <div className="bg-[#1a1c26] p-6 rounded-3xl border border-white/10 shadow-xl">
              ...
              <form onSubmit={handleWeightSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required type="number" step="0.01" 
                        className="w-full bg-black/30 border border-white/10 rounded-xl p-3 mt-1 outline-none focus:border-blue-400"
                ...
                <button type="submit" className="w-full bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2">
            ...
            <div className="bg-[#1a1c26] p-6 rounded-3xl border border-white/10 shadow-xl">
              ...
              <form onSubmit={handleHealthActionSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-[10px] text-gray-500 uppercase font-black">Event Type</label>
                  ...
                  <div>
                    <label className="text-[10px] text-gray-500 uppercase font-black">Dose</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="text-[10px] text-gray-500 uppercase font-black">Observations</label>
                ...
                <button type="submit" className="w-full bg-red-500/20 text-red-400 hover:bg-red-500/30 font-bold py-3 rounded-xl transition-all flex items-center justify-center gap-2 mt-2">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="bg-[#1a1c26] p-6 rounded-3xl border border-white/10 shadow-xl">
              ...
              <p className="text-xs text-gray-400 mb-2">Dates highlighted in red indicate administered vaccines.</p>
            ...
            <div className="bg-[#1a1c26] rounded-3xl border border-white/10 shadow-xl overflow-hidden">
              <div className="p-6 border-b border-white/5 bg-white/[0.02]">
                <h3 className="font-bold flex items-center gap-2"><Scale size={18} className="text-blue-400"/> Weight History Log</h3>
              </div>
                  ...
                  {weightHistory.map((record, index) => {
                    ...
                    return (
                      <tr key={record.id} className="hover:bg-white/[0.02]">
                        <td className="p-4 text-xs text-gray-400">{record.fecha}</td>
                        <td className="p-4 text-sm font-bold text-white">{record.peso} kg</td>
                        <td className="p-4 text-xs font-bold">
                          {prevRecord ? (
                            <span className={isGain ? 'text-ganadero-active' : 'text-red-400'}>
